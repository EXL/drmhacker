...
find /usr/data_resource/picture -type f -name "*.dcf*" | xargs -d '\n' -n1 sh -c '../drmhacker_magx "$0" `basename "$0" .dcf`'
find /usr/data_resource/sound/ringtones -type f -name "*.dcf*" | xargs -d '\n' -n1 sh -c '../drmhacker_magx "$0" `basename "$0" .dcf`'

# One-liners for DRM file extensions
find /usr/data_resource/picture -type f -name "*.drm*" | xargs -d '\n' -n1 sh -c '../drmhacker_magx "$0" `basename "$0" | sed -E 's/\.drm//g'`'
find /usr/data_resource/sound/ringtones -type f -name "*.drm*" | xargs -d '\n' -n1 sh -c '../drmhacker_magx "$0" `basename "$0" | sed -E 's/\.drm//g'`'
